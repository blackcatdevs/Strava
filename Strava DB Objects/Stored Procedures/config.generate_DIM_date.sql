-- Thanks to https://www.mssqltips.com/sqlservertip/4054/creating-a-date-dimension-or-calendar-table-in-sql-server/
-- If this is run more than once on the same day it adds one new record to DIM_Date
-- This is benign behaviour so will not fix it

CREATE PROCEDURE config.generate_DIM_date
...
DECLARE @StartDate DATE = COALESCE(
                                   DATEADD(DAY, 1, (SELECT MAX(TheDate) FROM reporting.DIM_date)), --subsequent runs
                                   (SELECT MIN(start_date) FROM Staging.Activities) --when run for the first time
                                   ),
@CutoffDate DATE ;

-- default is to add 1 month of new dates
IF @CutoffDate IS NULL SET @CutoffDate = DATEADD(MONTH, 1, GETDATE());
